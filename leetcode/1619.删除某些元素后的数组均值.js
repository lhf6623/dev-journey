...
var trimMean = function (arr) {
  const n = arr.length;
  const lengthAfterTrim = Math.floor(n * 0.05);
  arr.sort((a, b) => a - b);

  let nums = 0;
  for (let i = lengthAfterTrim; i < n - lengthAfterTrim; i++) {
    nums += arr[i];
  }
  return nums / (n - lengthAfterTrim * 2);
};
const test = [
  [[[1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3]], 2],
  [[[6, 2, 7, 5, 1, 2, 0, 3, 10, 2, 5, 0, 5, 5, 0, 8, 7, 6, 8, 0]], 4],
